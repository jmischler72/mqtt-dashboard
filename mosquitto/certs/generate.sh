...
# for the mosquitto3 TLS/mTLS dev broker.
#
...
echo "==> Generating dev TLS certificates for mosquitto3..."
...
# when connecting as mosquitto3:8883 from within docker-compose.
openssl genrsa -out server.key 2048
openssl req -new -key server.key -out server.csr \
  -subj "/CN=mosquitto3/O=Dev/C=US"
cat > server.ext <<'EOF'
subjectAltName=DNS:mosquitto3,DNS:localhost,IP:127.0.0.1
EOF
...
echo "  server.key/crt — Mosquitto3 server certificate (used by mosquitto)"
