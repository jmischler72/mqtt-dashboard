...
export default function PanelWrapper({
  ...
  const saveConfig = async (cfg: unknown, brokerId: string) => {
    closeConfigModal();
    try {
  ...
  const headerMeta = defaultBuildHeaderMeta(def, panelConfig);
  const isVisual = def?.isVisual ?? false;
  ...
  const renderPanel = () => {
    ...
    return (
      <PanelComponent
        panelId={panel.id}
        brokerId={panel.broker_id ?? ""}
        config={panel.config_json ?? {}}
  ...
  const renderConfigModal = () => {
    if (!showConfig || !def) return null;
    const cfg = (panel.config_json ?? {}) as Record<string, unknown>;
