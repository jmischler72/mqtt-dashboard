...
function buildTree(topics: string[]): Map<string, TreeNode> {
    ...
    for (const topic of topics) {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (!current.has(part)) {
                current.set(part, { name: part, fullPath: path, children: new Map(), isLeaf: false })
            }
...
function TreeNodeItem({ node, selectedTopic, flashTopics, onSelect, depth }: NodeProps) {
    ...
    return (
        <div>
            ...
            >
                {hasChildren ? (
                    <span className="text-base-content/50 w-3 shrink-0">{open ? '▾' : '▸'}</span>
                ) : (
                    <span className="w-3 shrink-0" />
                )}
                <span className={`font-mono truncate ${node.isLeaf && !hasChildren ? 'text-accent' : ''}`}>{node.name}</span>
            </div>
            {hasChildren && open && (
                <div>
...
export default function TopicTree({ topics, liveMessages, selectedTopic, onSelectTopic }: TopicTreeProps) {
    ...
    if (tree.size === 0) {
        return (
            <div className="flex items-center justify-center h-full text-base-content/40 text-sm p-4 text-center">
                No topics captured yet.<br />Messages will appear once the broker starts sending data.
            </div>
        )
    }
